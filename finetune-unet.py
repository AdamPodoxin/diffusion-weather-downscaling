...
if __name__ == "__main__":
    ...
    X_hr_train_subset = X_hr_train.sel(channel=WIND_DIR_AND_PRESSURE_CHANNELS)
    ...
    X_hr_val_subset = X_hr_val.sel(channel=WIND_DIR_AND_PRESSURE_CHANNELS)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        lr_batch_generator = generate_batches(X_lr_train_subset, batch_size=BATCH_SIZE)
        hr_batch_generator = generate_batches(X_hr_train_subset, batch_size=BATCH_SIZE)
        ...
        with torch.no_grad():
            lr_val_batch_generator = generate_batches(X_lr_val_subset, batch_size=BATCH_SIZE)
            hr_val_batch_generator = generate_batches(X_hr_val_subset, batch_size=BATCH_SIZE)
